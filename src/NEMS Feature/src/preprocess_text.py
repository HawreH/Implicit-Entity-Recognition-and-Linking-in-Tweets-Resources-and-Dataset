...
def iter_reviews(in_loc):
    file_list = ['%s\%s' % (in_loc, x) for x in os.listdir(in_loc) if x.endswith('.txt')]
    for file_name in file_list:
        for lines in io.open(file_name, encoding='utf8'):
...
def main(in_loc, out_dir, n_workers=4, load_parses=False):
    ...
    if load_parses:
        ...
        jobs = partition(2000, iter_reviews(in_loc))
